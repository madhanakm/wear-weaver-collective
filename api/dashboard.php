    ...
    <div class="main-content">
        ...
        try {
            ...
            if ($viewId && $page === 'contacts') {
                ...
                echo "  fetch('http://localhost/api/blog-api.php?path=admin/post/' + id)";
                echo "    .then(r => r.json())";
                echo "    .then(post => {";
                ...
                echo "  fetch('http://localhost/api/blog-api.php?path=delete/' + id, { method: 'DELETE' })";
                echo "    .then(() => location.reload());";
                echo "}";
                echo "function savePost(e) {";
                echo "  e.preventDefault();";
                echo "  const id = document.getElementById('postId').value;";
                echo "  const url = id ? 'http://localhost/api/blog-api.php?path=update/' + id : 'http://localhost/api/blog-api.php?path=create';";
                ...
                echo "      const previewUrl = data.url.startsWith('http') ? data.url : 'http://localhost/api/' + data.url;";
                echo "      document.getElementById('galleryImagePreview').innerHTML = '<img src=\"' + previewUrl + '\" style=\"max-width: 200px; height: auto; border-radius: 4px;\">';";
                echo "    } else {";
                ...
                if ($viewGallery) {
                    ...
                    echo "  fetch('http://localhost/api/products-api.php?path=delete-gallery/' + id, { method: 'DELETE' })";
                    echo "    .then(() => location.reload());";
                    echo "}";
                    echo "</script>";
                ...
                if (count($sliders) > 0) {
                    ...
                    foreach ($sliders as $row) {
                        ...
                        if ($imageUrl && strpos($imageUrl, 'http') !== 0) {
                            $imageUrl = 'http://localhost/api/' . $imageUrl;
                        }
                ...
                echo "  fetch('http://localhost/api/sliders-api.php?path=admin')";
                echo "    .then(r => r.json())";
                echo "    .then(data => {";
                ...
                echo "          imageUrl = 'http://localhost/api/' + imageUrl;";
                echo "        }";
                echo "        if (imageUrl) {";
                ...
                echo "  fetch('http://localhost/api/sliders-api.php?path=delete/' + id, { method: 'DELETE' })";
                echo "    .then(() => location.reload());";
                echo "}";
                ...
                echo "  const url = id ? 'http://localhost/api/sliders-api.php?path=update/' + id : 'http://localhost/api/sliders-api.php?path=create';";
                echo "  const method = id ? 'PUT' : 'POST';";
