...
<head>
    ...
    <link rel="icon" type="image/png" href="nsg_logo.png">
    ...
    <div class="main-content">
        ...
        try {
            ...
            if ($viewId && $page === 'contacts') {
                ...
                echo "  fetch('$API_BASE_URL/upload-image.php', {";
                echo "    method: 'POST',";
                echo "    body: formData";
                ...
                echo "  fetch('$API_BASE_URL/blog-api.php?path=admin/post/' + id)";
                echo "    .then(r => r.json())";
                echo "    .then(post => {";
                ...
                echo "  fetch('$API_BASE_URL/blog-api.php?path=delete/' + id, { method: 'POST' })";
                echo "    .then(() => location.reload());";
                echo "}";
                echo "function savePost(e) {";
                echo "  e.preventDefault();";
                echo "  const id = document.getElementById('postId').value;";
                echo "  const url = id ? '$API_BASE_URL/blog-api.php?path=update/' + id : '$API_BASE_URL/blog-api.php?path=create';";
                ...
                echo "  fetch('$API_BASE_URL/clients-api.php')";
                echo "    .then(r => r.json())";
                echo "    .then(data => {";
                ...
                echo "  fetch('$API_BASE_URL/clients-api.php?id=' + id, { method: 'POST' })";
                echo "    .then(r => r.json())";
                echo "    .then(data => {";
                echo "      if (data.success) {";
                echo "        location.reload();";
                ...
                echo "      } else {";
                echo "        let html = '<h3>Found ' + count + ' unused images:</h3><div style=\"display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 15px; margin: 20px 0;\">';";
                echo "        data.unused_files.forEach(file => {";
                echo "          html += '<div style=\"border: 1px solid #ddd; border-radius: 8px; overflow: hidden; text-align: center; padding: 10px;\"><img src=\"$API_BASE_URL/uploads/' + file + '\" style=\"width: 100%; height: 120px; object-fit: cover; border-radius: 4px; margin-bottom: 8px;\" onerror=\"this.style.display=&quot;none&quot;\"><div style=\"font-size: 12px; color: #666; word-break: break-all;\">' + file + '</div></div>';";
                echo "        });";
                echo "        html += '</div>';";
